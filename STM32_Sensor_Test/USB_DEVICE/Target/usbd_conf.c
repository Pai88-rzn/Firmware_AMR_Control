/**
  ******************************************************************************
  * @file           : usbd_conf.c
  * @brief          : Low level USB configuration for STM32F411 OTG FS
  ******************************************************************************
  */
...
void HAL_PCD_MspInit(PCD_HandleTypeDef* hpcd)
{
  ...
  if (hpcd->Instance == USB_OTG_FS)
  {
    ...
    /* Configure DM (PA11) and DP (PA12) in AF10 (OTG_FS) */
    ...
    /* Set USB interrupt priority (lower than FreeRTOS syscall max priority) */
...
{
  HAL_StatusTypeDef hal_status = HAL_PCD_EP_Open((PCD_HandleTypeDef*)pdev->pData, ep_addr, ep_mps, ep_type);
  return (hal_status == HAL_OK) ? USBD_OK : USBD_FAIL;
}
...
{
  HAL_StatusTypeDef hal_status = HAL_PCD_EP_Transmit((PCD_HandleTypeDef*)pdev->pData, ep_addr, pbuf, size);
  return (hal_status == HAL_OK) ? USBD_OK : USBD_FAIL;
}

USBD_StatusTypeDef USBD_LL_PrepareReceive(USBD_HandleTypeDef *pdev, uint8_t ep_addr, uint8_t *pbuf, uint32_t size)
{
  HAL_StatusTypeDef hal_status = HAL_PCD_EP_Receive((PCD_HandleTypeDef*)pdev->pData, ep_addr, pbuf, size);
  return (hal_status == HAL_OK) ? USBD_OK : USBD_FAIL;
}
